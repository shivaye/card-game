...
<head>
  ...
  <title><?=SITENAME;?> | Manage Reedeemed Points</title>
    ...
    <div class="content-wrapper">
      ...
      <div class="content-header">
        <div class="container-fluid">
          <div class="row mb-2">
            <div class="col-sm-6">
              <h1 class="m-0 text-dark">Manage Reedeemed Points</h1>
            </div>
            <div class="col-sm-6">
              <ol class="breadcrumb float-sm-right">
                <li class="breadcrumb-item"><a href="#">Home</a></li>
                <li class="breadcrumb-item active">Manage Reedeemed Points</li>
              </ol>
            </div>
          </div>
        </div>
              ...
              <div class="card-body">
                <table id="example1" class="table table-bordered table-striped">
                  ...
                  <tbody>
                    <?php
                    $i=1;
                      foreach(@$points as $points_row){
                    ?>
                    <tr>
                      <td><?=$i;?></td>
                      <td><?=$points_row['fname']?></td>
                      <td><?=$points_row['phone_number']?></td>
                      <td><?=$points_row['reedeem_point']?></td>
                      <td><?=$points_row['created_date']?></td>
                      <td><input type="text" name="remarks" placeholder="Remarks" class="form-control" id="remark_id<?=$points_row['point_id']?>" value="<?=RemoveSpecialChar($points_row['remarks'])?>"></td>
                       <?php
                      if($points_row['status']==1){
                        $class = "success";
                        $word = "Active";
                      }else{
                        $class = "danger";
                      ...
                      <td><i class="fa fa-refresh btn btn-success" onclick="update_reedeem('<?=$points_row['point_id']?>')"></i></td>
                    </tr>
                    <?php
                    $i++;
                      }
...
<script>
  ...
  function update_reedeem(id)
  {
    if(confirm('Are You Sure Want To Update User points'))
    {
      var remarks = $('#remark_id'+id).val();
      window.location.href='<?=base_url().'admin/inquiry/update_reedeem/';?>'+id+'/'+remarks;
    }
  }
</script>
</body>
