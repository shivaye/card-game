...
<head>
  ...
  <title><?=SITENAME;?> | Manage Product</title>
             ...
             <div class="card">
              <div class="card-header">
                <div class="card-tools">
                  <a class="btn btn-primary" href="<?=base_url().'admin/product/create';?>"><i class="fas fa-plus"></i> Create</a>
                </div>
              </div>
              <div class="card-body">
                <table id="example1" class="table table-bordered table-striped">
                  ...
                  <tbody class="row_position">
                    ...
                    <tr id="<?=$product_row['id'];?>">
                      ...
                      <td><a href="<?=base_url().'admin/product/edit/'.$product_row['id']?>" class="btn btn-info fa fa-edit"></a>  <a href="javascript:void(0)" onclick="deleteProduct('<?=$product_row['id']?>')" class="btn btn-danger fa fa-trash"></a></td>
                    </tr>
                    <?php
                    $i++;
                      }
...
<script type="text/javascript">
    ...
    function updateOrder(data) {
        $.ajax({
            url:"<?=base_url()?>admin/product/sortable",
            type:'post',
            data:{position:data},
            success:function(){
      ...
      function update_front(product_id)
  {
   if($('#front_id'+product_id).prop("checked") == true){
    var checks = 1;
  }
  else if($('#front_id'+product_id).prop("checked") == false){
    var checks = 0;
  }

  $.ajax({
    url:"<?=base_url()?>admin/product/front_id",
    type:'post',
    data:{"product_id":product_id,"front_id":checks},
    dataType:"html",
