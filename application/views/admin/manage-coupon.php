...
<head>
  ...
  <title><?=SITENAME;?> | Manage Coupon</title>
             ...
             <div class="card">
              <div class="card-header">
                <div class="card-tools">
                  <a class="btn btn-primary" href="<?=base_url().'admin/coupon/create';?>"><i class="fas fa-plus"></i> Create</a>
                </div>
              </div>
              <div class="card-body">
                <table id="example1" class="table table-bordered table-striped">
                  ...
                  <tbody>
                    <?php
                    $i=1;
                      foreach($allBanner as $banner_row){
                    ?>
                    <tr>
                      <td><?=$i;?></td>
                      <td><?=$banner_row['coupon_code']?></td>
                      <td><?=$banner_row['start_date']?></td>
                      <td><?=$banner_row['end_date']?></td>
                        <?php
                          if($banner_row['discount_type']==1){
                        ?>
                      <td><?=$banner_row['percentage']?> (%)</td>
                      <?php
                        }else{
                      ?>
                      <td><?=$banner_row['flat_amount']?> (Flat)</td>
                      <?php
                        }
                      ?>
                      ...
                      <td><a href="<?=base_url().'admin/coupon/edit/'.$banner_row['coupon_id']?>" class="btn btn-info fa fa-edit"></a>  <a href="javascript:void(0)" onclick="deleteCategory('<?=$banner_row['coupon_id']?>')" class="btn btn-danger fa fa-trash"></a></td>
                    </tr>
                    <?php
                    $i++;
                      }
...
<script>
  ...
  function deleteCategory(id)
  {
    ...
    {
      window.location.href='<?=base_url().'admin/coupon/delete/';?>'+id;
    }
  }
</script>
</body>
