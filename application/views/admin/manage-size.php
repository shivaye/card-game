...
<head>
  ...
  <title><?=SITENAME;?> | Manage Size</title>
             ...
             <div class="card">
              <div class="card-header">
                <div class="card-tools">
                  <a class="btn btn-primary" href="<?=base_url().'admin/size/create';?>"><i class="fas fa-plus"></i> Create</a>
                </div>
              </div>
              <div class="card-body">
                <table id="example1" class="table table-bordered table-striped">
                  ...
                  <tbody class="row_position">
                    <?php
                    $i=1;
                      foreach($getAllSize as $row){
                    ?>
                    <tr id="<?=$row['size_id'];?>">
                      <td><?=$i;?></td>
                      <td><?=$row['size']?></td>
                      <td><a href="<?=base_url().'admin/size/edit/'.$row['size_id']?>" class="btn btn-info fa fa-edit"></a>  <a href="javascript:void(0)" onclick="deleteCategory('<?=$row['size_id']?>')" class="btn btn-danger fa fa-trash"></a></td>
                    </tr>
                    <?php
                    $i++;
                      }
...
<script>
  ...
  {
    if(confirm('Are You Sure Want To Delete Size'))
    {
      window.location.href='<?=base_url().'admin/size/delete/';?>'+id;
    }
  }
</script>
</body>
