...
<head>
  ...
  <title><?=SITENAME;?> | Manage Newspaper</title>
    ...
    <div class="content-wrapper">
      ...
      <div class="content-header">
        <div class="container-fluid">
          <div class="row mb-2">
            <div class="col-sm-6">
              <h1 class="m-0 text-dark">Manage Newspaper</h1>
            </div>
            <div class="col-sm-6">
              <ol class="breadcrumb float-sm-right">
                <li class="breadcrumb-item"><a href="#">Home</a></li>
                <li class="breadcrumb-item active">Manage Newspaper</li>
              </ol>
            </div>
          </div>
        </div>
             ...
             <div class="card">
              <div class="card-header">
                <div class="card-tools">
                  <a class="btn btn-primary" href="<?=base_url().'admin/newspaper/create';?>"><i class="fas fa-plus"></i> Create</a>
                </div>
              </div>
              <div class="card-body">
                <table id="example1" class="table table-bordered table-striped">
                  ...
                  <tbody>
                    <?php
                    $i=1;
                      foreach($allnewspaper as $newspaper_row){
                    ?>
                    <tr>
                      <td><?=$i;?></td>
                      <td><?=$newspaper_row['newspaper_name']?></td>
                      <td><?php if($newspaper_row['image']!="" && file_exists('./public/uploads/newspaper/thumb/'.$newspaper_row['image'])){?>
                        <img src="<?=base_url().'public/uploads/newspaper/thumb/'.$newspaper_row['image']?>" alt="No image found" style="width: 150px; height: 150px;">
                        <?php
                          }else{
                        ?>
                        ...
                        ?></td>
                      <td><?=$newspaper_row['language']?></td>
                      <td><?=$newspaper_row['name'];?></td>
                      <?php
                        if($newspaper_row['status']==1){
                          $class = "success";
                          $word = "Active";
                        }else{
                          $class = "danger";
                      ...
                      <td><a href="<?=base_url().'admin/newspaper/edit/'.$newspaper_row['newspaper_id']?>" class="btn btn-info fa fa-edit"></a>  <a href="javascript:void(0)" onclick="deleteNewspaper('<?=$newspaper_row['newspaper_id']?>')" class="btn btn-danger fa fa-trash"></a></td>
                    </tr>
                    <?php
                    $i++;
                      }
...
<script>
  ...
  {
    if(confirm('Are You Sure Want To Delete Newspaper'))
    {
      window.location.href='<?=base_url().'admin/newspaper/delete/';?>'+id;
    }
  }
</script>
</body>
